...
from src.Domain.Song import Url
...
class PlaylistApplicationService:
    ...
    def addSong(self, arg_keyword: str, arg_url: str):
        ...
        for song in songs:
            ...
            exists = self.playlistService.hasTheSong(playlistID, song.songID)
    ...
    def addPlaylist(self, arg_keyword: str):
        ...
        # return message
        raise OriginalException(Messages.addPlaylistMessage(keyword))

    # 既存のプレイリストを登録する
    def registerPlaylist(self, arg_keyword: str, arg_url: str):
        ...
        # get playlist json
        playlistJson = json.load(request.urlopen(arg_url))

        # make songID list
        songIDs = []
        for songJson in playlistJson["songs"]:
            # get song
            song = self.songRepository.findByHash(songJson["hash"])
            if song is None:
                song = self.songFactory.generateByHash(songJson["hash"])
                self.songRepository.upsert(song)
        ...
        playlist = Playlist(
            ...
            description=PlaylistDescription(
                "This is generated by discordbot-BSPlaylistManager."),
            image=self.imageFactory.convertByImageFile(arg_url),
            songIDs=songIDs
        )
        ...
        # return message
        raise OriginalException(Messages.registerPlaylistMessage(keyword))
    ...
    def updateFileName(self, arg_keyword: str, arg_filename: str):
        ...
        # return message
        raise OriginalException(
            Messages.updatePlaylistInfoMessage(playlist.keyword))
    ...
    # 画像を更新する
    def updateImage(self, arg_keyword: str, arg_url: str):
    ...
    # プレイリストをダウンロードする
    def download(self, arg_keyword: str) -> Tuple[str, Dict]:
        ...
        # 更新したプレイリスト情報を用意する
        playlistJson = self.playlistService.convert2Json(playlist)

        # return message
        return (Messages.updatePlaylistInfoMessage(playlist.keyword), playlistJson)
